...
export class InvoicesService {
  ...
  async listInvoices(filters: InvoiceFiltersDto): Promise<ListInvoicesDto[]> {
    ...
    const cachedInvoices = (await this.cacheManager.get(
      JSON.stringify(where),
    )) as ListInvoicesDto[];

    if (cachedInvoices !== null)
      return await this.parseInvoiceAmounts(
        cachedInvoices,
  ...
  private async parseInvoiceAmounts(
    invoices: ListInvoicesDto[],
    ...
    for (const invoice of invoices) {
      if (targetCurrency && targetCurrency !== invoice.currency) {
        const conversionRate: number =
          conversionPairs[`${invoice.currency}_${targetCurrency}`];

        invoice.creditTotal = roundToTwoDecimals(
          invoice.creditTotal * conversionRate,
        );
        invoice.invoiceTotal = roundToTwoDecimals(
          invoice.invoiceTotal * conversionRate,
        );
        invoice.paymentTotal = roundToTwoDecimals(
          invoice.paymentTotal * conversionRate,
        );
      }
  ...
  private async getAllConversionPairs(): Promise<ConversionPairsInterface> {
    const cachedPairs = (await this.cacheManager.get(
      'conversionPairs',
    )) as ConversionPairsInterface;

    if (cachedPairs) return cachedPairs;

    const usdPairsPromise = this.fetchConversionRatesFromCurrency(
      CurrencyEnum.USD,
      [CurrencyEnum.CLP, CurrencyEnum.EUR],
    );

    const eurPairsPromise = this.fetchConversionRatesFromCurrency(
      CurrencyEnum.EUR,
      [CurrencyEnum.USD, CurrencyEnum.CLP],
    );

    const clpPairsPromise = this.fetchConversionRatesFromCurrency(
      CurrencyEnum.CLP,
      [CurrencyEnum.USD, CurrencyEnum.EUR],
    );

    const [usdPairs, eurPairs, clpPairs] = await Promise.all([
      usdPairsPromise,
      eurPairsPromise,
      clpPairsPromise,
    ]);
